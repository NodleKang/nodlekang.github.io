...
일반적으로 재개발에서 동호수는 같은 평형 신청 1순위 안에서 권리가액 순으로 가져갑니다.

1순위에서 떨어지면 남은 것 중에 랜덤 추첨이 됩니다.
...
1. 큰평수를 배정받으면 작은평수를 배정받을때 보다 프리미엄을 더 높게 받을수 있기 때문에 대다수 조합원들은 큰평수를 선호한다.
2. 조합원분양신청을 하게되면 원하는 평형을 1순위, 2순위, 3순위로 적게 되는데 25평(100개), 34평(50개), 45평(20개)까지 총3가지가 있다고 가정한다.
3. 조합원은 총100명으로 이중 권리가액(감정평가액X비례율)이 나름 높다고 생각되는 30명이 1순위로 45평, 2순위로 34평, 3순위로 25평을 썼다면 30명중 10명은 2순위로 밀리게 된다.
4. 2순위로 34평을 썼으나 1순위로 34평을 신청한 조합원이 50명이라면 이들에게 밀리면서 3순위로 가게된다.
5. 평형배정은 동일순위에서는 권리가액이 높은순으로 우선 배정받게 되지만 순위에서 밀리게 된다면 권리가액이 높아도 밀리게 된다.
6. 위는 예시일뿐이고 실제로는 평형도 훨씬 다양하고 설사 같은 평형이라도 A,B,C등등 타입을 달리하여 나오기 때문에 눈치싸움이 굉장히 치열하다.
7. 설령, 무허가주택을 소유하여 권리가액이 100만원이더라도 머리만 좋다면 수싸움에서 승리하여 중대형평형 배정도 가능할수 있다.
